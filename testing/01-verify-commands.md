...
Gunakan: Jalankan berurutan, screenshot hasil 
untuk dokumentasi UKK
...
Verifikasi di Router:
    ...
    Winbox → Log → Harus ada entry "FWD-DROP" 
    saat VLAN 20 ping ke VLAN 10
...
Test dari browser PC Admin:
    ...
    ✅ HARUS: Apache default page + icon gembok
    (warning self-signed = normal)
...
Cek port listening:
    ...
    ✅ HARUS: zabbix_agentd (10050), zabbix_server (10051), 
              apache2 (80)
...
| 2  | VLAN Config Router | /interface vlan print |
...
| 6  | HTTPS Web | Browser: https://www.lab-smk.xyz |
| 7  | Zabbix Dashboard | Host Router & Server Green + Graph |
...
Internet tidak jalan di Server?
    ...
    ip route show
    # Pastikan: default via 192.168.30.1
...
VLAN isolasi tidak bekerja?
    ...
    # Cek urutan rules: allow established harus paling atas
